...
import { RegisterStep2Data, RegisterStep3Data } from "@/schemas/cronos/g8.schemas"
...
export const resendCode = async (individual_id: string) => {
    try {
        const response = await http.post(`/v1/register/individual/resendsmscode`, { individual_id })
        return response.data
    } catch (error) {
...
export const registerStep3 = async (data: RegisterStep3Data & { file: File | Blob }) => {
  try {
